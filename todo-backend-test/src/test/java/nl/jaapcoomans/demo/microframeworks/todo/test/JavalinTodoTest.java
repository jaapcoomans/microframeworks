...
class JavalinTodoTest {
    ...
    @Container
    private static GenericContainer javalinContainer = new GenericContainer(javalinImage)
            .withExposedPorts(7000);

    private static String getBaseUrl() {
        return "http://" + javalinContainer.getContainerIpAddress() + ":" + javalinContainer.getFirstMappedPort();
    }

    private static String replacePort(String url) {
        return url.replaceFirst("[1-9][0-9]{1,4}", javalinContainer.getFirstMappedPort().toString());
    }
    ...
    void getApiRoot() {
        RestAssured.given()
                ...
                .header("Access-Control-Allow-Headers", HttpHeaders.CONTENT_TYPE)
